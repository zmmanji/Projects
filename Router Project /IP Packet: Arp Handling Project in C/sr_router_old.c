...
void handleIpPacket(struct sr_instance* sr, uint8_t* frame, uint8_t frameLen, struct sr_if* interface) {
	...
	if (ipHeader->ip_p == htons(ip_protocol_icmp) && isDestinedForRouter(sr, ipHeader)) {
		...
		{
			printf("\t\t...examining entry in routing table...\n");
			printf("\t\t\t entry mask: %x\n", routingTable->mask.s_addr);
			printf("\t\t\t packet dst_ip: %d\n", ipHeader->ip_dst);
			printf("\t\t\t entr dst_ip: %d\n", routingTable->dest.s_addr);
		...
		if (sendingIfaceName == NULL) {
			fprintf(stderr, "\t\t>>Could not find suitable outgoing interface: Network Unreachable - sending icmp type=3 msg.\n");
			...
			printf("\t\t>>Found an outgoing interface\n");
			...
			if ((ipHeader->ip_ttl - 1) == 0) {
				fprintf(stderr, "\t\t>>Packet's Time to Live expired in transit.\n");
				...
				printf("\t\t>>Packet TTL is ok: searching arp cache...\n");
				...
				if (arpEntry != NULL) {
					printf("\t\t>>Found an entry for the interface connected to the dst IP\n");
					...
					printf("\t\t>>Did not find an interface corresponding to the dst ip\n");
...
/* @todo: use our own logic later: we're using the key for now */
void sr_handlepacket(struct sr_instance* sr,
	...
	printf(">>>sr_handlepacket: Received packet of length %d\n",len);
    ...
    if(len < sizeof(sr_ethernet_hdr_t)) {
		printf("\t>>Frame is invalid: ethernet header length is too short.\n");
        return;
	}
    ...
    if(ethertype(packet) == ethertype_arp) {
		printf("\t>>Frame contains an ARP packet, calling handleArpPacket()\n");
	...
	/* if frame contains an IP Packet */
		...
		printf("\t>>Frame contains an IP packet: check if its destined for router...\n");
        ...
        if(destIsRouter) {
			printf("\t>>Frame is destined for router, calling handleIpPacket()\n");
            handleIpPacket(sr, packet, len, iface);
        } else {
            fprintf(stderr,"\t>>Frame is not destined for the router, frame dropped.\n");
		}
    }
    else {
        fprintf(stderr, "\t>>Frame contains neither ARP nor IP packet, frame dropped.\n");
	}
}/* -- sr_handlepacket -- */
